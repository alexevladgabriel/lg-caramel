...
server.get("/", async (request: FastifyRequest, reply: FastifyReply) => {
  return reply
    .status(200)
    .send({ message: "https://github.com/KittensAreDaBest/caramel" });
});
...
server.register(async function (fastify) {
  fastify.get(
    ...
    async (connection: WebSocket, request: FastifyRequest) => {
      ...
      console.log(
        `[${new Date()}][LATENCY] WebSocket connected from ${remoteIp}`
      );
      ...
      connection.on("close", () => {
        console.log(
          `[${new Date()}][LATENCY] WebSocket disconnected from ${remoteIp}`
        );
      });
    }
  );
...
server.post("/lg", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  switch (validation.type) {
    ...
    case "traceroute":
      ...
      const traceroute = spawn.spawnSync("traceroute", [
        "-w",
        "1",
        "-q",
        "1",
      ...
      return reply.status(200).send({
        data:
          traceroute.stdout.toString().length > 0
            ? traceroute.stdout.toString()
            : traceroute.stderr.toString(),
      });
    ...
    case "bgp":
      ...
      const bgp = spawn.spawnSync("birdc", [
        "-r",
        "sh",
        "ro",
        "all",
